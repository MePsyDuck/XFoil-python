...
def restart_xfoil(xfoil, p_index, foil_name, parsed_file, restarts):
    if restarts < max_restarts:
        ...
        raise Exception('{0:4d}: {1:20s} was restarted too many times.'.format(p_index, foil_name))


def run_sequence(xfoil, p_index, foil_name, parsed_file, start, end, step_size_, smaller_step_size_):
    restarts = 0
...
def run_xfoil(p_index, parsed_file):
    ...
    if load_file(xfoil, parsed_file, foil_name):
        try:
            xfoil = run_sequence(xfoil, p_index, foil_name, parsed_file, 0, 20, step_size, smaller_step_size)
            run_sequence(xfoil, p_index, foil_name, parsed_file, 0, -20, -step_size, -smaller_step_size)
        ...
        print('{0:4d}: File: {1:20s} processed. Took: {2:5f} seconds'.format(p_index, foil_name, end - start_time))
